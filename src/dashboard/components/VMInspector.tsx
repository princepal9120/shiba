  ...
  if (!activeRun) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-[#f6f4ed]">
        ...
        <h3 className="text-base font-semibold text-[#171932] mb-1">No Virtual Machine Selected</h3>
        <p className="text-xs text-[#6a6f63] max-w-sm">
    ...
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#f6f4ed] text-[#222320]">
      ...
      <div className="h-14 border-b border-black/[0.08] bg-[#f6f4ed]/95 backdrop-blur-md px-4 lg:px-6 flex items-center justify-between shrink-0 gap-3">
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-mono border border-black/[0.08] bg-[#fffef8]">
            <span
              className={`w-2 h-2 rounded-full ${
                isLive
                  ? "bg-[#0000a8] animate-pulse"
                  : activeRun.status === "completed"
                  ? "bg-[#15803d]"
                  : "bg-[#fb2c36]"
              }`}
            />
            <span className="text-[#6a6f63]">{statusLabel(activeRun.status)}</span>
            {sandboxInfo?.placementId ? (
              <span className="text-[#6a6f63] opacity-70">({sandboxInfo.placementId})</span>
            ) : null}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-xs bg-[#fffef8] hover:bg-[#eae8e1] border border-black/[0.08] text-[#222320] font-medium py-1.5 px-3 rounded-lg transition-colors flex items-center gap-1.5 shadow-sm"
          ...
          <button
            ...
            className="text-xs bg-[#fffef8] hover:bg-[#eae8e1] border border-black/[0.08] text-[#6a6f63] hover:text-[#171932] p-1.5 rounded-lg transition-colors"
      ...
      <div className="bg-[#f6f4ed]/70 border-b border-black/[0.08] px-4 lg:px-6 py-2.5 flex flex-wrap items-center justify-between gap-y-2 gap-x-4 text-xs font-mono text-[#6a6f63]">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
          <span className="text-[#171932] font-semibold flex items-center gap-1.5">
          ...
          {activeRun.publishPullRequest ? (
            <span className="text-[#0000a8] bg-[#dceafa]/40 border border-[#9cbce2]/40 px-1.5 py-0.2 rounded text-[10px]">
              PR Enabled
            </span>
          ) : null}
        </div>
        <div className="text-[11px] text-[#6a6f63]">
          Started {formatTimeAgo(activeRun.createdAt)}
        </div>
      </div>

      {/* TASK DESCRIPTION SUMMARY */}
      <div className="bg-[#f6f4ed] border-b border-black/[0.08] px-4 lg:px-6 py-2 flex items-center justify-between text-xs">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-[#6a6f63] shrink-0 font-medium">Task:</span>
          <span className="text-[#222320] truncate font-sans">{activeRun.task}</span>
        </div>
        {activeRun.summary ? (
          <span className="text-[11px] text-[#0000a8] shrink-0 font-mono hidden md:inline-block">
            Summary available
          </span>
        ) : null}
      </div>
      ...
      {/* SUB-TABS NAVIGATION */}
      <div className="border-b border-black/[0.08] bg-[#f6f4ed] px-4 lg:px-6 flex items-center gap-1 shrink-0 overflow-x-auto">
        <button
          type="button"
          ...
          className={`px-3 py-2.5 text-xs font-medium border-b-2 transition-colors flex items-center gap-1.5 whitespace-nowrap ${
            subTab === "diff"
              ? "border-[#0000a8] text-[#0000a8] font-semibold bg-[#eae8e1]/30"
      ...
      {/* SUB-TAB CONTENTS */}
      <div className="flex-1 overflow-y-auto p-4 lg:p-6 bg-[#fffef8]">
        {/* 1. CHANGES & DIFF TAB */}
        {subTab === "diff" ? (
          <div className="flex flex-col gap-4 max-w-5xl mx-auto">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-semibold text-[#171932]">Work in Progress Diff</h3>
            ...
            {activeRun.summary ? (
              <div className="mt-4 border border-black/[0.08] rounded-xl bg-[#f6f4ed] p-4">
                ...
                <pre className="font-mono text-xs text-[#222320] whitespace-pre-wrap leading-relaxed">
                  {activeRun.summary}
                </pre>
              </div>
            ) : null}
        ...
        {subTab === "files" ? (
          <div className="flex flex-col xl:flex-row gap-4 h-full min-h-[480px]">
            ...
            <div className="w-full xl:w-72 bg-[#f6f4ed] border border-black/[0.08] rounded-xl p-3 flex flex-col shrink-0">
              <div className="flex items-center justify-between pb-2 mb-2 border-b border-black/[0.08] text-xs font-mono">
                ...
                <button
                  ...
                  className="text-[#0000a8] hover:text-[#1c1cc8] text-[11px]"
                  title="Up directory"
                >
                  ../
                </button>
              ...
              {fileLoading && fileList.length === 0 ? (
                <div className="text-xs text-[#6a6f63] p-4 text-center">Loading files...</div>
              ) : fileList.length === 0 ? (
                <div className="text-xs text-[#6a6f63] p-4 text-center">
                    ...
                    <button
                      ...
                      className={`text-left px-2 py-1.5 rounded flex items-center gap-2 truncate transition-colors ${
                        selectedFile === entry.path
                          ? "bg-[#dceafa]/60 text-[#1c1cc8] border border-[#9cbce2]/60"
                          : "hover:bg-[#eae8e1] text-[#222320]"
                      }`}
                    >
                      <svg
            ...
            <div className="flex-1 bg-[#f6f4ed] border border-black/[0.08] rounded-xl flex flex-col overflow-hidden">
              <div className="h-10 border-b border-black/[0.08] px-4 bg-[#f6f4ed] flex items-center justify-between text-xs font-mono">
                ...
                {fileContent !== null ? (
                  <button
                    ...
                    className="text-[11px] text-[#6a6f63] hover:text-[#171932]"
                  >
                    Copy content
                  </button>
                ) : null}
              ...
              <div className="flex-1 overflow-auto p-4 bg-[#fffef8] font-mono text-xs">
                {fileContent !== null ? (
                  <pre className="whitespace-pre text-[#222320] leading-relaxed">
                    {fileContent}
                  </pre>
                ) : (
                  <div className="h-full flex items-center justify-center text-[#6a6f63] italic">
        ...
        {subTab === "terminal" ? (
          <div className="flex flex-col gap-4 max-w-5xl mx-auto">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold text-[#171932]">Live Virtual Machine Terminal</h3>
                <p className="text-xs text-[#6a6f63]">
              ...
              <div className="flex flex-wrap gap-1.5 text-xs font-mono">
                {["git status", "git log -n 3", "ls -la /workspace", "cat package.json"].map((cmd) => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded bg-[#fffef8] border border-black/[0.08] hover:bg-[#eae8e1] text-[#222320] transition-colors"
                  >
                    {cmd}
                  </button>
                ))}
            ...
            <div className="bg-[#f6f4ed] border border-black/[0.08] rounded-xl overflow-hidden shadow-2xl flex flex-col min-h-[380px]">
              <div className="h-8 bg-[#f1efe6] border-b border-black/[0.08] px-3 flex items-center justify-between text-xs font-mono text-[#6a6f63]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-2 text-[#171932]">sandbox@cloudflare-vm:~/workspace</span>
                </div>
                <span>shiba-bash v1</span>
              </div>

              {/* Output Scroll Area */}
              <div className="flex-1 p-4 overflow-y-auto font-mono text-xs bg-[#fffef8] flex flex-col gap-4 max-h-[500px]">
                {terminalHistory.length === 0 ? (
                  <div className="text-[#6a6f63] italic">
                    Terminal ready. Click a preset above or enter a bash command below to execute inside the container.
                  </div>
                ) : (
                  terminalHistory.map((item, idx) => (
                    <div key={idx} className="flex flex-col gap-1 border-b border-black/[0.08]/60 pb-3">
                      <div className="flex items-center justify-between text-[#6a6f63] text-[11px]">
                        <div className="flex items-center gap-1.5 text-[#0000a8] font-semibold">
                          <span>$</span>
                          <span className="text-[#171932]">{item.command}</span>
                        </div>
                        <span>exit code: {item.exitCode} · {item.time}</span>
                      </div>
                      {item.stdout ? (
                        <pre className="text-[#222320] whitespace-pre-wrap pl-3 border-l-2 border-[#0000a8]/30">
                          {item.stdout}
                        </pre>
                      ) : null}
                      {item.stderr ? (
                ...
                {isPending ? (
                  <div className="text-[#0000a8] flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-[#0000a8] animate-ping" />
              ...
              <form
                ...
                className="h-12 border-t border-black/[0.08] bg-[#fffef8] px-3 flex items-center gap-2"
              >
                <span className="text-[#0000a8] font-mono text-xs font-bold">$</span>
                <input
                  type="text"
                  required={true}
                  ...
                  className="flex-1 bg-transparent border-none text-xs font-mono text-[#171932] focus:outline-none placeholder:text-[#6a6f63]/50"
                  disabled={isPending}
                />
                <button
                  type="submit"
        ...
        {subTab === "preview" ? (
          <div className="flex flex-col gap-4 max-w-5xl mx-auto h-full">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="text-sm font-semibold text-[#171932]">Cloudflare Sandbox Web Preview</h3>
                <p className="text-xs text-[#6a6f63]">
              ...
              <div className="flex items-center gap-2 font-mono text-xs">
                <span className="text-[#6a6f63]">Port:</span>
                {[3000, 5173, 8080].map((port) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded border transition-colors ${
                      previewPort === port
                        ? "bg-[#dceafa]/70 border-[#0000a8] text-[#1c1cc8] font-semibold"
                        : "bg-[#fffef8] border-black/[0.08] text-[#6a6f63] hover:text-[#171932]"
                    }`}
                  >
                    {port}
                  </button>
            ...
            <div className="border border-black/[0.08] bg-[#f6f4ed] rounded-xl p-5 flex flex-col gap-3">
              <div className="flex items-center gap-2 text-sm font-semibold text-[#171932]">
            ...
            <div className="flex-1 border border-black/[0.08] rounded-xl bg-[#f6f4ed] flex flex-col min-h-[350px] overflow-hidden">
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-[#f6f4ed]">
                ...
                <h4 className="text-sm font-semibold text-[#171932] mb-1">Sandbox Web Application</h4>
        ...
        {subTab === "processes" ? (
          <div className="flex flex-col gap-4 max-w-5xl mx-auto">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-semibold text-[#171932]">Container System & Security</h3>
                <p className="text-xs text-[#6a6f63]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-[#f6f4ed] border border-black/[0.08] rounded-xl p-4 flex flex-col gap-1">
                ...
                <span className="text-base font-semibold text-[#171932] flex items-center gap-2">
              ...
              <div className="bg-[#f6f4ed] border border-black/[0.08] rounded-xl p-4 flex flex-col gap-1">
                ...
                <span className="text-base font-semibold text-[#171932] font-mono">
            ...
            {/* Processes Table */}
            <div className="bg-[#f6f4ed] border border-black/[0.08] rounded-xl p-5 flex flex-col gap-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-[#171932] font-mono">
              ...
              {sandboxInfo?.processes && sandboxInfo.processes.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="w-full text-left font-mono text-xs">
                    <thead>
                      <tr className="border-b border-black/[0.08] text-[#6a6f63]">
                        <th className="pb-2">PID</th>
                        <th className="pb-2">Command</th>
                        <th className="pb-2">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#eae8e1]/50">
                      {sandboxInfo.processes.map((proc, i) => (
                        <tr key={i}>
                          <td className="py-2 text-[#0000a8]">{proc.pid}</td>
                          <td className="py-2 text-[#171932]">{proc.command}</td>
                          <td className="py-2 text-[#6a6f63]">{proc.status || "running"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="text-xs text-[#6a6f63] italic">
