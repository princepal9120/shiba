...
export function DiffViewer({ diff, runId }: DiffViewerProps): JSX.Element {
  ...
  return (
    <div className="flex flex-col gap-2 w-full mt-2">
      {/* Diff Header Bar */}
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 bg-[#fffef8] border border-black/[0.08] rounded-t-xl shadow-sm text-xs">
        <div className="flex items-center gap-2 font-mono">
          <span className="text-[#6a6f63] font-medium">Unified Diff</span>
          {stats.files > 0 ? (
            <span className="bg-[#f6f4ed] text-[#222320] border border-[#e0ded5] rounded px-1.5 py-0.5 text-[11px]">
        ...
        <div className="flex items-center gap-2">
          <Tooltip content={copied ? "Diff copied to clipboard!" : "Copy unified diff to clipboard"} side="top">
            <button
              type="button"
              onClick={copyDiff}
              className="text-[11px] font-sans px-2.5 py-1 rounded bg-[#f6f4ed] hover:bg-[#e0ded5] text-[#222320] border border-black/[0.08] transition-colors flex items-center gap-1.5"
            >
              {copied ? (
                <>
          ...
          <Tooltip content="Download raw git patch file" side="top">
            <button
              ...
              className="text-[11px] font-sans px-2.5 py-1 rounded bg-[#f6f4ed] hover:bg-[#e0ded5] text-[#6a6f63] hover:text-[#222320] border border-black/[0.08] transition-colors flex items-center gap-1"
            >
      ...
      {/* Code Block */}
      <pre
        className="font-mono text-xs leading-[1.45] bg-[#f6f4ed] border border-t-0 border-black/[0.08] rounded-b-xl p-3.5 -mt-2 max-h-96 overflow-auto whitespace-pre block w-full shadow-inner"
