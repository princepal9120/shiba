...
| --- | --- | --- | --- |
| Fenced execution identities (`execution.generation` carried across I/O, re-checked in the writing transaction via `tx.fenced`; stale writer throws `Superseded` and drops) | COPY | Landed in `src/runs.ts` (`DelegatedRun.generation`, `RunStore.transition(..., expectedGeneration)` returns null on mismatch) and `src/agents/orchestrator.ts` (fenced `running` transition + fenced `finish()`). Prevents a late child result overwriting terminal state. | — |
| `outcome_unknown` — acknowledged-then-vanished work is indeterminate, not failed | COPY | `src/runs.ts` terminal `"unknown"` status + `errorCode: "outcome_unknown"` on reclaim; `src/run-errors.ts` wire projection `status: "unknown"`. | — |
| Idempotency reservations (`canonicalJSON` fingerprint of accepted input, `session-kinds.ts` `idempotency` kind) | ADAPT | Landed as delivery-ID dedupe in `src/index.ts` (`gh-delivery:<x-github-delivery>` via `/internal/dedupe`). Theirs fingerprints request content; ours dedupes transport redeliveries. | A write path that can be retried with different delivery IDs needs content-fingerprint idempotency instead. |
| `statusToTurnCode` — HTTP status → error-code mapping shared by all harnesses | COPY | `statusToRunCode` in `src/run-errors.ts` (401/403→authentication_error, 404→resource_not_found, 408→request_timeout, 429→rate_limit_exceeded, 503/529→server_overloaded, other 5xx→server_error, other 4xx→invalid_request, null→internal_error). | — |
| Tagged error union (`Data.TaggedError` classes generated from the `DEFINITE` table, `errors.ts`) | ADAPT | `RunErrorCode` closed union + `RUN_ERROR_DEFS` table + `classifyRunError` in `src/run-errors.ts`. Table-driven user copy is theirs; classes and `catchTag` are not needed without Effect. | If Effect is adopted (see Effect-TS row), the union becomes `Data.TaggedError` subclasses. |
...
| Boundary error squashing (one wire projection for API + notifications) | COPY | `runErrorWire(code)` in `src/run-errors.ts` produces `{status, code, userMessage}` for API responses and Slack posts; raw text stays on the record post-`redactSecrets`. | — |
...
| Harness/sandbox split (separate supervisor + harness containers per session) | SKIP | One sandbox per run; the egress boundary (`src/egress.ts`) keeps provider keys and `GITHUB_TOKEN` out of the container. | A repo-code-vs-agent isolation incident (untrusted repo code reaching the agent's credential surface) flips this. |
...
| Multi-tenancy (environments, per-customer isolation) | SKIP | ai-intern is self-hosted, single-tenant by design. | A hosted product shape flips it. |
...
- "Landed" files are cited in the Rationale column and checked by
  ...
  `test/cf-patterns-doc.test.ts`): COPY rows must name existing files; every
  SKIP/ADAPT row must carry its flip criterion.
- Error classification is intentionally deterministic. The repo already uses
  TypeSafe (`src/typesafe.ts`) where an LLM judgment is warranted; the
