...
AI Coworker connects directly to your team Slack workspace. Developers can trigger coding tasks, inspect plans, and grant approval directly in thread conversations.
...
>
> **AI Coworker**: on it — queued a claude code run for princepal9120/shiba. tap approve and i'll start.
>
> [ **Approve** ]   [ **Reject** ]

The coworker replies like a colleague, not a task machine — `apps/backend/src/slack-persona.ts` holds the voice spec and every message helper. Once approved it runs the task end to end, posts short progress updates in the same thread, and finishes with a colleague-style summary (what changed and the PR link).
