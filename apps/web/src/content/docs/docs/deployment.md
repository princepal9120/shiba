...
**Do not treat this as production until T10 is dated in `VERIFICATION.md`.** Put Cloudflare Access on the Worker, with a bypass only for callers the Worker authenticates itself: `/api/slack/events`, `/api/slack/command`, `/api/slack/interact`, `/api/github/webhook`, `/mcp`, and `/api/automations/*/trigger`. `pnpm run bootstrap` creates both Access apps for you. Read [Security](/docs/security/) and [Readiness](/docs/readiness/) first.
...
These commands change your Cloudflare account. `pnpm run deploy` builds first, then runs `alchemy deploy`. Wrangler uses `./Dockerfile` for the image and `./public` for assets; the dashboard is at `/app/` and docs are at `/docs/`. Static missing paths use `404-page` rather than an SPA catch-all.
