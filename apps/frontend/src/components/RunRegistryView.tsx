...
export function RunRegistryView({
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-[var(--background)] text-slate-900">
      {/* Top Header */}
      <div className="border-b border-[var(--border)] bg-[var(--card)]/95 backdrop-blur-md px-6 lg:px-8 py-5 flex flex-wrap items-center justify-between gap-4 shrink-0">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <h2 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2.5">
              <span>Run Registry & Workspaces</span>
            </h2>
            <span className="bg-[var(--secondary)] text-slate-600 text-[11px] font-bold px-2.5 py-0.5 rounded-full border border-[var(--border)]">
              {stats.total} TOTAL
            </span>
          </div>
        ...
        <div className="flex items-center gap-2.5">
          <button
            type="button"
            onClick={onRefresh}
            className="text-xs bg-white hover:bg-[var(--secondary)] border border-[var(--border)] text-slate-700 font-semibold py-2 px-3.5 rounded-lg transition-all shadow-sm flex items-center gap-2"
          >
      ...
      <div className="p-6 lg:p-8 flex-1 overflow-y-auto flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3.5">
          {/* Total Runs */}
          <div className="bg-white p-4 rounded-xl border border-[var(--border)] shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-[var(--secondary)] flex items-center justify-center text-slate-500 border border-[var(--border)]">
        ...
        <div className="bg-white rounded-2xl border border-[var(--border)] shadow-sm overflow-hidden flex flex-col min-h-[480px]">
          {/* Controls bar */}
          <div className="p-4 border-b border-[var(--border)] flex flex-wrap items-center justify-between gap-3 bg-[var(--secondary)]/60">
            <div className="flex bg-white border border-[var(--border)] p-1 rounded-lg shadow-2xs">
            ...
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                ...
                className="w-full bg-white text-xs text-slate-800 border border-[var(--border)] rounded-lg pl-9 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 placeholder:text-slate-400 shadow-2xs"
              />
            </div>
          </div>
          ...
          {filteredRuns.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
              <div className="mb-5 relative">
                ...
                <div className="absolute -bottom-1.5 -right-1.5 w-8 h-8 bg-white border-2 border-[var(--border)] rounded-full flex items-center justify-center text-blue-600 shadow-xs">
            ...
            <div className="p-4 flex flex-col gap-3">
              {filteredRuns.map((run) => {
                ...
                const sColor = statusColors[run.status] || "text-slate-600 border-[var(--border)] bg-[var(--secondary)]";
                const repoName = parseRepoName(run.repoUrl);

                return (
                  <div
                    key={run.runId}
                    className="border border-[var(--border)] rounded-xl bg-white overflow-hidden transition-all hover:border-[var(--border)] shadow-2xs"
                  >
                    <button
                      type="button"
                      ...
                      className="w-full text-left p-4 flex items-center justify-between cursor-pointer hover:bg-[var(--secondary)]/70 select-none gap-3"
                    >
                    ...
                    {isExpanded ? (
                      <div className="p-4 border-t border-[var(--border)] bg-[var(--secondary)]/60 flex flex-col gap-4">
                        ...
                        <pre className="font-sans text-xs text-slate-800 whitespace-pre-wrap break-words bg-white p-3 rounded-lg border border-[var(--border)] leading-relaxed shadow-2xs">
                          {run.task}
                        </pre>

                        {/* Summary */}
                        {run.summary ? (
                          <div className="border border-[var(--border)] rounded-lg p-3 bg-white shadow-2xs">
                        ...
                        {/* Action Bar */}
                        <div className="flex items-center gap-2 pt-2 border-t border-[var(--border)]/80">
                          ...
                          <button
                            type="button"
                            className="text-xs bg-white hover:bg-[var(--secondary)] border border-[var(--border)] text-slate-600 hover:text-slate-900 font-semibold py-1.5 px-3 rounded-md transition-colors shadow-2xs"
