...
function makeEnv(): Env & { r2: Map<string, FakeR2Object> } {
  const stubs = new Map<string, { fetch: (r: Request) => Promise<Response> }>();
  ...
  const typed = env as unknown as Env & { r2: Map<string, FakeR2Object> };
  typed.r2 = r2;
  return typed;
}
...
describe("handleInboundEmail", () => {
  ...
  it("threads a reply onto the original via In-Reply-To", async () => {
    ...
    await handleInboundEmail(
      makeMessage(rfc822({ From: "b@example.com", To: REGISTERED, Subject: "Re: question?", "Message-ID": "<reply@x>", "In-Reply-To": "<orig@x>" }, "second")).message,
      env,
    );
    const emails = await listEmails(env);
  ...
  it("captures attachments into R2 keyed emailId/att-N with metadata", async () => {
    const env = makeEnv();
    await registerMailbox(env);
    const raw =
      `From: a@example.com\nTo: ${REGISTERED}\nSubject: with file\nMIME-Version: 1.0\n` +
    ...
    const obj = env.r2.get(`${email!.id}/att-0`);
