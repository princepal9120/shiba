...
5. **Store Secrets with Wrangler**
   ...
   ```bash
   pnpm wrangler secret put SLACK_BOT_TOKEN --config backend/wrangler.jsonc
   pnpm wrangler secret put SLACK_SIGNING_SECRET --config backend/wrangler.jsonc
   ```
...
Once approved, AI Coworker spins up an isolated sandbox container, runs the changes, and posts the resulting GitHub PR link directly in the thread.
