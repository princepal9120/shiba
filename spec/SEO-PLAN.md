...
**Why build it:** developers need a task-to-reviewable-change workflow without giving a hosted vendor unchecked repository access. Shiba's distinctive boundary is exact-input human approval before sandbox startup, account-owned Cloudflare infrastructure, provider credentials held at AI Gateway egress, and inspectable output. The trust and ownership story is more credible than “fully autonomous engineer” copy.
...
## Cloudroom Core comparison (source snapshot: 2026-09-26)

| Question | Shiba, current repo | Cloudroom Core, published docs |
| --- | --- | --- |
| First job | Approval-gated task → isolated run → progress, diff, optional PR | Durable coding sessions, prompt queue, replay and recovery |
| Host | Cloudflare Worker, Agents/DOs, Sandbox/Containers in user's account | Rust service, one Linux VM, separate PostgreSQL |
| Execution | New sandbox per delegated run; no start before exact-input approval | Codex/Pi processes in an agent account; [setup says they run without approval prompts](https://github.com/davidondrej/cloudroom-core/blob/main/docs/setup.md) |
| Credentials | Real provider credentials stay outside the container at AI Gateway egress | Harness inference credentials live in the agent account; [harness docs explicitly say they are not isolated from its tools](https://github.com/davidondrej/cloudroom-core/blob/main/docs/harnesses.md) |
| Continuity | Retained runs and task-level automation, not native conversation recovery | Native session history, queued prompts, restart recovery and replay |
| Maturity caveat | Local run verified to gateway model call; cloud deploy not proven here | README calls it pre-release and notes some architecture components are planned |

Cloudroom is stronger on durable session recovery; Shiba is designed around a gated, ephemeral Cloudflare task workflow. Neither project should be described as uniquely “the first.” Sources: [Cloudroom README](https://github.com/davidondrej/cloudroom-core), [session lifecycle](https://github.com/davidondrej/cloudroom-core/blob/main/docs/session-lifecycle.md), [harnesses](https://github.com/davidondrej/cloudroom-core/blob/main/docs/harnesses.md), and this repo's `spec/GOAL.md`, `PLAN.md`, `VERIFICATION.md`.
...
| Compare alternatives | Future `/compare/cloudroom-core/` **only after product verification** | Cloudroom Core alternative | Honest comparison + sources |
...
4. **Days 61–90 — compare and update.** After a proven cloud run, publish evidence-backed comparisons with Cloudroom Core and hosted agents. Recheck competitor docs before each revision. Update positioning if Shiba's actual workflow changes.
